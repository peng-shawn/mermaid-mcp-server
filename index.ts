...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    log("error", `Received request: ${name} with args: ${JSON.stringify(args)}`);
    ...
    if (name === "generate") {
      if (testing) {
        return {
          content: [
            ...
            { type: "image", data: stageImage, mimeType: "image/png" },
          ],
          isError: false,
        };
      }
      ...
      //   base64Image= stageImage;
      // }
      log("Base64 image:", base64Image);
      return {
        content: [
          {
            type: "text",
